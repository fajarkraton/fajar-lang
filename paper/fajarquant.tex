...
\midrule
Adaptive rotation ($b{=}3$, $d{=}128$) & MSE improvement & \textbf{88\%} \\
Adaptive rotation ($b{=}3$, $d{=}64$) & MSE improvement & \textbf{81\%} \\
Fused attention ($N{=}16$K) & Memory saved & 4~MB $\to$ 64~B \\
Hierarchical ($N{=}4$K) & Bit savings & \textbf{65.3\%} \\
KV cache ($d{=}64$, $b{=}3$) & Compression & \textbf{6.4$\times$} \\
...
FajarQuant achieves 55--88\% lower MSE than TurboQuant on structured (low-rank) data by exploiting KV cache structure through adaptive PCA rotation. The improvement is data-dependent: structured data with concentrated variance yields the highest gains, while uniform random data shows minimal improvement at $b{=}1$. Combined with fused codebook attention ($O(N \cdot d) \to O(1)$ extra dequant memory) and hierarchical bit allocation (up to 65\% fewer bits at $N{=}4096$, 12\% at $N{=}256$), FajarQuant enables practical LLM inference on embedded devices.
