...
\begin{tabular}{lcccc}
\toprule
Configuration & \makecell{Key MSE\\($b{=}2$)} & \makecell{MSE $\Delta$\\vs Turbo} & \makecell{Dequant\\Memory} & \makecell{Bit\\Savings} \\
\midrule
...
$+$ Hierarchical (Full FQ) & $\mathbf{8.72 \times 10^{-4}}$ & $-4.9\%$ & $O(2^b)$ & $48.7\%$ \\
\bottomrule
\end{tabular}
\end{table}
