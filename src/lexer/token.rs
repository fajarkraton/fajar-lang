...
pub enum TokenKind {
    ...
    /// `dyn`
    Dyn,
    /// `comptime` — compile-time evaluation block/modifier
    Comptime,
...
impl fmt::Display for TokenKind {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            TokenKind::AtMessage => write!(f, "@message"),
...
pub static ANNOTATIONS: LazyLock<HashMap<&'static str, TokenKind>> = LazyLock::new(|| {
    ...
    m.insert("message", TokenKind::AtMessage);
...
mod tests {
    ...
    fn token_kind_display_all_types() {
        let cases: Vec<(TokenKind, &str)> = vec![
            ...
            (TokenKind::F16Type, "f16"),
            (TokenKind::Bf16Type, "bf16"),
        ];
        for (kind, expected) in cases {
            assert_eq!(format!("{kind}"), expected, "Display for {kind:?}");
        }
    ...
    fn token_kind_display_all_annotations() {
        let cases: Vec<(TokenKind, &str)> = vec![
            ...
            (TokenKind::AtSafe, "@safe"),
            (TokenKind::AtUnsafe, "@unsafe"),
            (TokenKind::AtFfi, "@ffi"),
            (TokenKind::AtTest, "@test"),
            (TokenKind::AtShouldPanic, "@should_panic"),
            ...
            (TokenKind::AtNoStd, "@no_std"),
            (TokenKind::AtReprC, "@repr_c"),
            (TokenKind::AtReprPacked, "@repr_packed"),
            (TokenKind::AtSimd, "@simd"),
            (TokenKind::AtSection, "@section"),
            (TokenKind::AtInterrupt, "@interrupt"),
            (TokenKind::AtMessage, "@message"),
            (TokenKind::AtShared, "@shared"),
            (TokenKind::AtRequires, "@requires"),
            (TokenKind::AtEnsures, "@ensures"),
            (TokenKind::AtInvariant, "@invariant"),
    ...
    fn token_kind_display_special_literals() {
        assert!(
            !format!(
                "{}",
                TokenKind::FStringLit(vec![FStringPart::Literal("hello".into())])
            )
            .is_empty()
        );
