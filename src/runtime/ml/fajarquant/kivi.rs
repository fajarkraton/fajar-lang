...
pub fn kivi_quantize_keys(keys: &Array2<f64>, bits: u8) -> KiviQuantizedKeys {
    ...
    KiviQuantizedKeys { indices, scales, zeros, bits }
}
...
pub fn kivi_quantize_values(values: &Array2<f64>, bits: u8) -> KiviQuantizedValues {
    ...
    KiviQuantizedValues { indices, scales, zeros, bits }
}
...
fn turboquant_mse(data: &Array2<f64>, bits: u8) -> f64 {
    use crate::runtime::ml::turboquant::{create_config, quant_mse, dequant_mse};
...
fn fajarquant_adaptive_mse(data: &Array2<f64>, bits: u8) -> f64 {
    use super::adaptive::{CalibrationBuffer, compute_pca_rotation, encode_adaptive, decode_adaptive, adaptive_codebook};
    use crate::runtime::ml::turboquant::create_config;
    ...
    for row in data.rows() {
        ...
        let mse: f64 = v.iter().zip(decoded.iter())
            .map(|(a, b)| (a - b) * (a - b))
            .sum::<f64>() / (d as f64);
        total_mse += mse;
        count += 1;
...
mod tests {
    ...
    fn kivi_value_quantize_roundtrip() {
        ...
        let recon = kivi_dequantize_values(&qv);
        let mse = (&data - &recon).mapv(|x| x * x).mean().unwrap();
        assert!(mse < 0.01, "4-bit KIVI value MSE should be small, got {mse}");
    }
    ...
    fn three_way_comparison_runs() {
        ...
        assert!(result.fq_vs_turbo > 0.0,
            "FajarQuant should improve over TurboQuant on low-rank data, got {}%", result.fq_vs_turbo);
    }
