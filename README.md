...
[![Release v33.7.0](https://img.shields.io/badge/release-v33.7.0_Stage--1_Use--Site_Closure-brightgreen)](https://github.com/fajarkraton/fajar-lang/releases/tag/v33.7.0)
...
| Metric | Value |
|--------|-------|
| Release | **v33.7.0 "Stage-1 Use-Site Closure"** (2026-05-05) — closes the "struct/enum DECL hollow" gap from v33.6.0: struct literal `Point{x:10,y:20}` + field access `p.x`, enum variant use `Color::Green`, `for` loop with `start..end` range. Plus binop atom-detection bug fix surfaced by P22. Test suite 17 → 22 (P18-P22 NEW), all PASS in 0.10s. Headline composability test P21: accumulator with struct literal mutation in for loop = 15. Cumulative ~7h vs plan 5-15d. Source: `docs/SELFHOST_FJ_PHASE_{0..10}_FINDINGS.md`. |
