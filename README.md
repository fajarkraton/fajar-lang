...
[![Release v33.8.0](https://img.shields.io/badge/release-v33.8.0_match_expression-brightgreen)](https://github.com/fajarkraton/fajar-lang/releases/tag/v33.8.0)
...
| Metric | Value |
|--------|-------|
| Release | **v33.8.0 "match expression"** (2026-05-05) — closes the borderline-case from v33.7.x deferred list. `match subject { pat => body, _ => default }` over enum variants and integer literals; composes as regular expr atom (let-rhs, return-arg, inside arithmetic). Codegen via GCC statement expression (`({ int64_t _match_N; if(...) _match_N = ...; _match_N; })`). Test suite 22 → 31 (incl. v33.7.x extensions). All PASS in 0.21s. Cumulative ~8h vs plan 5-15d. Source: `docs/SELFHOST_FJ_PHASE_{0..11}_FINDINGS.md`. |
