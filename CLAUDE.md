...
|---|---|---|
| **V29.P3.P6** "NX Triple Closure" | 2026-04-16 | FajarOS **V26 B4.2 security triple 3/3 COMPLETE** — 5 commits in ~1.48h (-19% vs 1.83h revised budget, -35% vs 2.27h original). H3 hypothesis (`nx_enforce_data_pages()` mis-classifies PD[1]) matched via P0 static analysis ALONE — no P1 NX walker needed (saved 0.55h). Root cause: kernel `.text` spans 0x101000-0x248297 (2.285 MB), straddles PD[0]+PD[1]; loop started at `pd_idx=1` marked PD[1] NX, causing silent triple-fault (no EXC marker — #PF → handler page NX → #DF → #DF handler also NX → CPU halt). Fix: single-line edit `pd_idx=1 → pd_idx=2` in `security.fj:236`. Prevention: `make test-security-triple-regression` 6-invariant gate (adds `NX_ENFORCED=0x800` EFER.NXE check). Commits: `17a42b0` plan, `b3c20b8` findings, `6bf1c2c` decision, `540743b` fix, `ac07fc9` regression gate. Docs: `V29_P3_P6_NX_{PLAN,FINDINGS,DECISION}.md`. Full V29.P3 session total: **13 commits, ~3.9h for V26 B4.2 SMAP+NX dual closure**. |
| **V29.P3** "SMAP Re-enable" | 2026-04-16 | FajarOS **V26 B4.2 SMAP blocker CLOSED** — 8 commits in ~2.4h (-9% vs budget). H2 hypothesis (non-leaf USER bits in SMAP AND-chain) empirically matched via new `pte_walk_find_u_leaks_full` 4-level walker that found PML4[0]+PDPT[0] leaks invisible to leaf-only walker (leaks at 0x71027+0x72027, flags PRESENT\|WRITABLE\|USER\|ACCESSED). Fix: extend `strip_user_from_kernel_identity()` to clear USER bit on both non-leaf entries; result `PTE_LEAKS_FULL=0x0`. SMEP+SMAP now co-exist cleanly — kernel boots through [VFS] + nova>. **NX deferred as V29.P3.P6** (separate silent triple-fault at println(16), isolated via bisect SMEP+SMAP-works/SMEP+SMAP+NX-hangs, TODO tagged in `main.fj:151`). Prevention: `make test-smap-regression` 5-invariant gate (leaf walker, non-leaf walker, no PLKNL, no EXC:8, nova> reached). Commits: `a521d4c`..`34426af`. Plan: `fajaros-x86/docs/V29_P3_SMAP_PLAN.md`, findings: `V29_P3_SMAP_FINDINGS.md` §10, decision: `V29_P3_SMAP_DECISION.md`. |
