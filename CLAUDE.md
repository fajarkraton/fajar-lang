...
| **FajarQuant Phase E** (bilingual ID+EN ternary kernel-context LLM, Tier 1+2 only — Tier 3 deferred to Phase F) | **`~/Documents/fajarquant/docs/FJQ_PHASE_E_BILINGUAL_KERNEL_PRODUCTION_PLAN.md`** v1.9 (post-E2.4 sync) — solo execution mode; ~10-month calendar (Tier 3 deferred); laptop-only RTX 4090 hardware. **E0 + E1 + E2.0 + E2.4 all CLOSED.** Bilingual corpus v1.0: 25.67 B tokens at 60:40 ID:EN. Q5 baseline measured (val_loss(ID)=2.68, val_loss(EN)=4.73, ratio=1.77×). **E2.4 produced honest NEGATIVE result** (`outlier_global_reduction = −82.13`, gate FAIL by 82×) — balanced_calib demoted to infra-diagnostic; future-work `FJQ_PHASE_F_TAX_VERTICAL_ROADMAP.md` §4.1 F.5 (post-training PTQ-style calibration). **Next: E2.1 Hadamard rotation** on `block.attn.o_proj` per QuaRot/SpinQuant. Companions: `FJQ_PHASE_E_E2_FINDINGS.md` v1.1 (E2.0 closure), `FJQ_PHASE_E_E2_4_FINDINGS.md` v1.4 (E2.4 closure), `FJQ_PHASE_E_E2_4_C_METRIC_SPEC.md` v1.0 (metric spec), `FJQ_PHASE_E_E2_BILINGUAL_CALIB_DECISION.md` v1.0 (E2.4 demote decision), `FJQ_PHASE_E_TAXPRIME_DATASET_SPEC.md` v1.2 (Phase F header). |
...
*CLAUDE.md Version: 31.0+V31.C.TRACKB+V31.E0+V31.E1+V31.E2.4+V31.E2.PathA | v31.0.0 release "Phase D + Track B" 2026-04-23 consolidates 8-day catch-up across compiler+OS+quant | **V31.E2.PathA "Submission-ready" 2026-04-27** — fajarquant Path A Weeks 1-4 ENTIRELY COMPLETE in 15 commits (`4852f3d` scope-decision through `f493ba5` arXiv tarball); manuscript `paper/intllm/intllm.tex` 2435 LOC + 20-page compiled PDF + 5 figures + refs.bib v1.0 (15 verified citations) + verify_intllm_tables.py --strict 32/32 PASS + standalone-verified arXiv tarball 77 KB; cumulative ~24h actual vs ~19 days est (−91%); founder actions (ORCID/Zenodo/arxiv/review/upload) per `docs/ARXIV_SUBMISSION.md` remain | V31.E2.4 "balanced_calib FAIL" 2026-04-27 — Phase E2.4 done with HONEST NEGATIVE RESULT (outlier_global_reduction = −82.13); plus E2.1 Hadamard FAIL same day (val_loss regression +0.12 nat); both demoted to Phase F.5/F.6 future-work | V31.E1 "Phase E1 fully closed" 2026-04-27 — bilingual corpus v1.0 25.67 B tokens (60:40); Q5 baseline ratio 1.77× | V31.E0 "Phase E E0 closure" 2026-04-25/26 — Phase E pre-flight 100% closed | V31.C Track B "Training Interruption-Safety" 2026-04-22 — §6.11 rule + 5-layer defence | V30.TRACK4 "FS Roundtrip" — FajarOS Nova v3.7.0 | V30.GEMMA3 "Foundation (Path D)" — v3.6.0 | V29.P1 "Compiler Enhancement" — @noinline lexer + 5-layer prevention | V27.5 baseline — 7,626 lib + 2,575 integ + 16 V27.5 E2E + 14 doc tests | §6.8+§6.9+§6.10+§6.11 Rules*
*Last Updated: 2026-04-27 (Path A submission-ready)*
