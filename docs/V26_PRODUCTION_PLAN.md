...
| C1.0 | **Single-model dry run (NEW Phase A lesson):** extract any model with **5 prompts only** (not 50), run 3-way comparison, sanity check MSE deltas in expected range. Validates pipeline before committing 12 GPU hours. ✅ DONE 2026-04-11 with cached `google/gemma-4-E2B` (40s wall time, 9 numerical claims match paper within tolerance — Mistral 7B download blocked by broken IPv6 routing + slow IPv4 to HF CDN) | `cd ~/Documents/fajarquant && python3 scripts/extract_kv_cache.py --model google/gemma-4-E2B --num-prompts 5 --output data/kv_cache/gemma_dryrun && python3 scripts/run_comparison.py --data data/kv_cache/gemma_dryrun --num-prompts 5 --bits 2,3,4` produces comparison_results.json with FajarQuant winning Keys at all bit widths | ✅ DONE — Gemma 4 E2B path validated; Mistral path deferred until network |
