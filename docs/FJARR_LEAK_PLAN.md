...
| **D** | Linear-types-lite (move-only `_FjArr`; double-use is a compile error; explicit `clone()` for sharing) | Type system: `[T]` is affine; analyzer rejects use-after-move; codegen emits `free` at last-use | ✅ **Best fit**. `@kernel` mode = same affine type system minus heap allocator. Composes by subtraction. Aligns with Compass §6.2 Hylo-style mutable value semantics. | Largest — analyzer change + codegen change. Needs SE024 use-after-move. | Zero overhead; identical to manual free. | Medium-risk — existing self-host source uses pass-by-value; some sites need `.clone()`. |
...
| 18.D.1 | (if D) Affine `[T]`; SE024 UseAfterMoveArray; `.clone()` builtin | `src/analyzer/*.rs`, `stdlib/codegen.fj` (preamble +_fj_arr_clone) | (a) lib 7,629 PASS (b) new SE024 test (c) phase17 PASS after rebase | +30% |
